...
from osprey.server.deepstream.spotmanager import SpotManager
...
class DynamicRTSPPipeline:
    ...
    def _require_playing(self) -> None:
        # Wait (bounded) for the state to settle rather than polling with a
        # 1-ns timeout — a fresh pipeline may still be finishing its async
        # transition to PLAYING when the first add_source arrives.
        ret, state, pending = self._pipeline.get_state(5 * Gst.SECOND)
        if state == Gst.State.PLAYING:
            return
        # Surface the ACTUAL state/return so startup failures are diagnosable.
        # ret=FAILURE + state=READY means an element failed to start (e.g. a
        # missing native dep such as nvtracker's libmosquitto.so.1) — not that
        # start() was never called.
        raise RuntimeError(
            f"Pipeline is not running (get_state={ret.value_nick}, "
            f"state={state.value_nick}, pending={pending.value_nick}). "
            "An element failed to reach PLAYING — check the server log for "
            "GStreamer/nvinfer/nvtracker errors."
        )
...
def main():
    ...
    from osprey.server.core.settings import settings
